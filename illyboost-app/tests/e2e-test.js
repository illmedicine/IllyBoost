...
async function runTests() {
  ...
  try {
    ...
    assert.strictEqual(response.status, 200, 'Expected status 200');
    assert(Array.isArray(response.body), 'Expected array response');
    assert.strictEqual(response.body.length, 0, 'Expected no agents initially');
    
    log.pass('Agents endpoint returns empty array initially');
    testsPassed++;
  } catch (e) {
  ...
  try {
    ...
    const agentFound = agentsResponse.body.some(a => a.id === agentId);
    assert(agentFound, 'Agent not registered');
  ...
  // Test 13: State Persistence - Agent Disconnect/Reconnect
  try {
    log.test('State Persistence: Agent Reconnection');
    
    const oldAgentId = agentId;
    const oldState = await httpRequest('GET', '/rows');
    ...
    // Row URLs should still be persisted
    const afterDisconnect = await httpRequest('GET', '/rows');
    const row1After = afterDisconnect.body.find(r => r.id === 1);
    assert.strictEqual(row1After.url, 'https://example.com', 'URL persistence failed');
    
    // Reconnect agent
    agentWS = await connectWS(AGENT_WS_PORT);
    const newAgentId = 'test-agent-reconnect-' + Math.random().toString(36).substr(2, 9);
    ...
    const afterReconnect = await httpRequest('GET', '/agents');
    const newAgentFound = afterReconnect.body.some(a => a.id === newAgentId);
    assert(newAgentFound, 'Reconnected agent not registered');
    
    agentId = newAgentId; // Update for cleanup
    log.pass('State persisted and agent reconnection handled');
    testsPassed++;
  } catch (e) {
